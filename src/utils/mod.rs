...
pub const DEFAULT_DATA_DIR: &str = "/var/lib/zenobox";
...
pub fn get_runc_bin() -> String {
    if let Ok(val) = std::env::var("ZENOBOX_RUNC") {
        return val;
    }
    ...
    let dest_dir = Path::new(&home).join(".zenobox/bin");
    let _ = fs::create_dir_all(&dest_dir);
    let dest_path = dest_dir.join("runc");
    if !dest_path.exists() {
        const RUNC_BYTES: &[u8] = include_bytes!("../../bin/runc-linux-amd64");
...
pub fn runc_exec(args: &[&str]) -> io::Result<std::process::Output> {
...
pub fn get_data_dir() -> String {
    if let Ok(dir) = std::env::var("ZENOBOX_DATA_DIR") {
        return dir;
    }
    ...
    if is_writable {
        ...
        let fallback_path = if let Ok(home) = std::env::var("HOME") {
            format!("{}/.zenobox/data", home)
        } else {
            "./data/zenobox".to_string()
        };

        static LOGGED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
        if !LOGGED.load(std::sync::atomic::Ordering::Relaxed) {
            eprintln!("⚠ [zenobox] Default directory {} is not writable. Falling back to local directory: {}", DEFAULT_DATA_DIR, fallback_path);
...
pub fn container_dir(data_dir: &str, id: &str) -> PathBuf {
    Path::new(data_dir).join("containers").join(id)
}

pub fn bundle_dir(data_dir: &str, id: &str) -> PathBuf {
    container_dir(data_dir, id).join("bundle")
}

pub fn rootfs_dir(data_dir: &str, id: &str) -> PathBuf {
    bundle_dir(data_dir, id).join("rootfs")
}

pub fn state_file(data_dir: &str, id: &str) -> PathBuf {
    container_dir(data_dir, id).join("state.json")
}

pub fn log_path(data_dir: &str, id: &str) -> PathBuf {
    container_dir(data_dir, id).join("console.log")
}

pub fn is_overlay_mounted(mount_point: &str) -> bool {
...
pub fn run_privileged_status(cmd: &str, args: &[&str]) -> io::Result<std::process::ExitStatus> {
...
pub fn run_privileged_output(cmd: &str, args: &[&str]) -> io::Result<std::process::Output> {
    let is_root = unsafe { libc::getuid() == 0 };
    if is_root {
        Command::new(cmd)
            .args(args)
...
pub fn run_cmd_status_silent(cmd: &str, args: &[&str]) -> io::Result<std::process::ExitStatus> {
    Command::new(cmd)
        .args(args)
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null())
